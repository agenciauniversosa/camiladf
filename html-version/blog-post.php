<?php
// Ordenados do mais recente para o mais antigo (mesma ordem do blog.php)
$posts = [
  'ilegalidade-busca-domiciliar-hc-960442' => [
  ...
  'justa-causa-trancamento-acao-penal-provas-ilicitas' => [
  ...
  'prisao-preventiva-regime-semiaberto-antecipacao-pena' => [
  ...
  'sumula-691-prisao-domiciliar-maes-gestantes' => [
  ...
  'superacao-sumula-691-stf-habeas-corpus' => [
...
$nextSlug = $currentIndex < count($slugs) - 1 ? $slugs[$currentIndex + 1] : null;
$prevPost = $prevSlug ? $posts[$prevSlug] : null;
$nextPost = $nextSlug ? $posts[$nextSlug] : null;

$pageTitle = $post['title'];
$pageDescription = $post['content'][0];
...
?>
  ...
  <article class="section-container">
    <div class="post-article">
      <div class="reveal">
        <a href="blog.php" class="post-back">
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg>
          Blog
        </a>

        <div class="post-meta">
          <span class="label-sm"><?php echo $post['category']; ?></span>
          <span><?php echo $post['date']; ?> · <?php echo $post['readTime']; ?></span>
        </div>

        <h1 class="heading-xl post-title"><?php echo $post['title']; ?></h1>
      </div>

      <div class="reveal post-body" data-delay="0.15">
        <?php foreach ($post['content'] as $i => $paragraph): ?>
          <p class="body-md"><?php echo $paragraph; ?></p>
        <?php endforeach; ?>
      </div>

      <!-- Share -->
      <div class="share-bar">
        <span class="label-sm">Compartilhar</span>
        <div class="share-icons">
          <a href="https://wa.me/?text=<?php echo urlencode($post['title']); ?>" target="_blank" rel="noopener noreferrer" class="share-btn" aria-label="WhatsApp">
          ...
          <a href="https://www.linkedin.com/sharing/share-offsite/?url=<?php echo urlencode('https://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']); ?>" target="_blank" rel="noopener noreferrer" class="share-btn" aria-label="LinkedIn">
            <svg viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
          </a>
          <a href="https://twitter.com/intent/tweet?text=<?php echo urlencode($post['title']); ?>" target="_blank" rel="noopener noreferrer" class="share-btn" aria-label="X">
      ...
      <nav class="post-nav">
        <?php if ($prevPost): ?>
          <a href="blog-post.php?slug=<?php echo $prevSlug; ?>" class="post-nav-link">
            <span class="nav-label label-sm">
            ...
            <span class="nav-title"><?php echo $prevPost['title']; ?></span>
          </a>
        <?php else: ?>
          <div></div>
        <?php endif; ?>
        <?php if ($nextPost): ?>
          <a href="blog-post.php?slug=<?php echo $nextSlug; ?>" class="post-nav-link next">
            <span class="nav-label label-sm">
              Próximo
            ...
            <span class="nav-title"><?php echo $nextPost['title']; ?></span>
          </a>
        <?php else: ?>
          <div></div>
        <?php endif; ?>
      ...
      <div class="post-cta">
        <p class="body-sm">Precisa de orientação sobre este tema?</p>
        <a href="index.php#contato" class="btn-primary">
          Fale Conosco
